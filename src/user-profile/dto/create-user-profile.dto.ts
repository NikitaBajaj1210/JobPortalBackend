...
export class CreateUserProfileDto {
  ...
  career_info: SeaServiceDto[];

  @ApiProperty({ type: [LegalDependentDTO] })
  @ArrayMaxSize(4, { message: 'You can add up to 4 legal dependents only.' })
  @ValidateNested({ each: true })
  @Type(() => LegalDependentDTO)
  ...
  @IsNotEmpty({ message: 'given_name cannot be empty' })
  @IsString({ message: 'given_name must be a valid string' })
  @Validate(IsNotWhitespace, {
    message: 'given_name cannot contain only whitespace',
  })
